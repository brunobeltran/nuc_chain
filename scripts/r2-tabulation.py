from multiprocessing import Pool
from pathlib import Path
import itertools

import numpy as np
import pandas as pd

from nuc_chain import fluctuations as wlc
from nuc_chain import geometry as ncg

#WARNING: change the file name format string if you change the simulation to
# use exponentially distributed chains instead, or add/remove kwargs like
# random_phi
mus = np.arange(39, 57)
sigmas = [0]
unwraps = [20]

def save_r2s(param, force_override=False):
    mu, sigma, unwrap = param
    file_name = f'csvs/r2/r2-fluctuations-homogenous-link-mu_{mu}-sigma_{sigma}_{unwrap}unwraps-random_phi.csv'
    if Path(file_name).exists() and not force_override:
        return
    Path(file_name).touch()
    dff = wlc.tabulate_r2_heterogenous_fluctuating_chains_by_variance(25, 7500, [sigma], mu=mu, unwraps=unwrap, random_phi=True)
    dff.to_csv(file_name)
    # for geometrical "box" case
    # file_name = f'csvs/r2/r2-geometrical-homogenous-link-mu_{mu}-sigma_{sigma}_{unwrap}unwraps-random_phi.csv'
    # dfg = ncg.tabulate_r2_heterogenous_chains_by_variance(100, 7500, [sigma], mu=mu, unwraps=unwrap)

pool_size=31
with Pool(processes=pool_size) as p:
    p.map(save_r2s, itertools.product(mus, sigmas, unwraps))
# for link in links:
#     save_r2s(mean_linker_length)
