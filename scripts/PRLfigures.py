...
def plot_fig2a():
    #R^2 vs. Rmax for homogenous kuhn length
...
def plot_fig2c():
    ...
    fig, ax = plt.subplots(figsize=(col_size, 0.42*col_size))
    plt.xlabel('Fixed linker length (bp)')
    plt.ylabel('Kuhn length (nm)')
    ...
    plt.tick_params(left=True, right=False, bottom=True, length=4)
    plt.savefig('plots/PRL/fig2c_kuhn_length_in_nm_31to51links_0unwraps.pdf')

def plot_fig3_exp_variance(ax=None):
    ...
    ax.plot(mug, kuhnsf, 'o', markersize=1, label='Fluctuations', color=teal_flucts)
    #dashed line at 100 nm
    ax.plot(np.linspace(0, max(mug), 1000), np.tile(100, 1000), '--', lw=0.75, label='Bare WLC', color=teal_flucts)
    #homogenous kuhn lengths faded in background
    ax.plot(mug, kuhns_homo, color=teal_flucts, alpha=0.5, lw=0.75, label='Homogenous')
    ...
    #plt.savefig('plots/PRL/fig3_kuhn_exponential.pdf')

def plot_fig4_kuhn_length_vs_variance(sigmas=np.arange(0, 11), ax=None):
...
def plot_fig4_inset(sigmas=np.arange(0, 41)):
    ...
    #entire figure
    ax.plot(sigmas, kuhnsf41, '-o', markersize=8, label='Fluctuations', color='#387780')
