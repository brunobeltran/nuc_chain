...
%TODO how full model is related
% to kinked chain~\cite{wiggins2005}, ....
%TODO also note force response of our full polymer has previously been discussed
% in~\cite{popov2005}.
...
%TODO reincorporate this (at end of intro?)
...
%Detailed computational models of individual chromatin chains with random
%    linker lengths have suggested that heterogeneity in nucleosome spacing
%    leads to largely disordered chromatin architecture~\cite{woodcock1993,
...
In this paper, we extend our analytical theory to incorporate arbitrarily spaced
    kinks, and use it to investigate the effects of nucleosome spacing
    on chromatin structure.
We first show that chains with delta-distributed linker lengths (periodic
    nucleosome spacing), vary sensitively as a function of average
    linker length $\meanli$.
As the entropy of the linker length distribution increases, the
    chromatin fiber asymptotically approaches a universal model that depends
    only on $\meanli$, and whose statistics change slowly with $\meanli$.
We use chains where linkers are chosen over a small, fixed range ($L_i \sim
    \mu \pm \sigma$) to show that this universal behavior is achieved for even
    small amounts of variability in the distribution of $L_i$.
Looping probabilities and Kuhn length as a function of $\meanli$ are reported for
    realistic nucleosome spacings.
Finally, we discuss features of chromatin that could be added to our model
    to bridge the gap between our bottom-up, analytical approach, and the
    detailed simulations that dominate the field~\cite{bascom2018}.

% We show that with even small amounts of linker length heterogeneity, the
%     elasticity of the chromatin fiber becomes only a function of the average
...
%     the organization of genetic material, but also the creation of
%     functional chromatin loops.
%DEEPTI: mobility of locus might be a stretch
...
%BRUNO: "from the bottom up" feels wrong, "bottom-up" is a well-known descriptor
    %for types of models (meaning they are based on first principles)
We believe that our results can provide theoretical guidance for bottom-up
    coarse-grained simulations of large-scale chromatin
    organization~\cite{macphersonInPress}.
...
At zero temperature, where the linkers are rigid rods, each linker
    (respectively, nucleosome) can be thought of as a fixed translation
    (rotation).
Since a rotation and translation specify an element of the Euclidean group,
    Chasles's theorem implies that iterating the same linker length will
    lead to a fiber where the nucleosomes are arranged in a helix.
Because the angle $\phi$ between linkers depends on the linker length, the shape
    of the helix formed by each homogenous chain depends on the linker length,
    as shown in Figure~\ref{fig:homo-kuhn-zoom}.
%TODO: add the example structures back in

We compute the Kuhn lengths of the homogenous nucleosome chain as function of
    linker length in Figure~\ref{fig:homo-kuhn} by fitting the long-chain limit
    of $\RR$.
...
% We that this corresponds exactly to the Kuhn length of the approriate discrete
%     helical wormlike chain, as first considered in~\cite{yamakawa1976}.
