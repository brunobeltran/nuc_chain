...
First, we examine the illustrative case of homogenous chains with fixed linker
lengths.
%here's where the chain becomes described only by the kuhn length
%only issue with this problem is that the length scale becomes Gaussian at
%different points depending on the chain configuration. going to delay this
%discussion until we make universality point
%In the long-length regime, $b$ characterizes the
% variance of the end-to-end distribution of a Gaussian chain.
...
In Fig 2a, we plot the Kuhn length of the kinked chain along with select minimum
energy structures of the chain for various linker lengths.
...
However, as can be seen in FIG 2a, the Kuhn length is well explained by how
...
superhelix of nucleosomes (as a consequence of Chasles' theorem).
The rise per basepair of the helix is determined by the spherical angles
$\theta$ and $\phi$ connecting adjacent linkers.
%we already discuss this in model section and in figure caption
% As shown in Figure~\ref{fig:entry-exit}, $\theta$ depends solely on the
% structure of the nucleosome and on the number of base pairs bound to the
% nucleosome.
Since $\phi$ depends linearly on the linker length due to the intrinsic helicity
of DNA, the rise/bp of the superhelix is \SI{10.5}{\basepair} periodic,
matching the DNA's helical periodicity (see Supp. Fig. XX).
...
temperature configuration. A larger rise/bp corresponds to a more processive
chain with a higher Kuhn length, whereas a smaller rise/bp creates a more
compact and therefore more flexible structure with a lower Kuhn length.

This geometrical explanation causes the Kuhn lengths of the fluctuating chains
to be extremely sensitive to the particular choice of linker length. As a
result, we hypothesized that introducing modest amounts of heterogeneity in the
linker length could significantly affect the geometric and physical organization of
heterogenous nucleosome chains.
...
%While the full theory is required to compute exact values for the Kuhn length
%(see Supp Fig), %as is the long linker limit of kuhn length goes in supp.
%not sure how the above phrase is referring the long linker limit figure?
...
%% what determines zero temperature picture for het chains, how do zero temp and
% fluct pictures compare
...
of the chain interpolates between a helix at $\sigma = 0$ to an increasingly
...
% This is evident in FIG, where we show how Kuhn lengths of the zero-temperature
% and fluctuating chains change as
% $\sigma$ increases.
As a result, the zero-temperature chain is equivalent to a modified ``hindered
rotating chain'' in which the bond angle $\theta$ is fixed, but the angle $\phi$
is random but correlated to the length of the rigid linkers. The Kuhn
length of this hindered rotating chain allows us to quantify our geometric notion of ``processivity'', and we show in
FIG how it qualitatively predicts the Kuhn length of the fluctuating chains.
This stands in contrast to models based on averaging the thermal fluctuations of
individual homogenous chains to compute the heterogenous chain's Kuhn
length, which fail dramatically (Supp FIG). 
We conclude that for realistic
...
As can be seen in Inlet figure, the functional
dependece of the Kuhn length on $\sigma$ is that of a dampened harmonic
oscillator. The decay rate is smaller in the fluctuating case than in the purely
geometrical picture due to thermal fluctuations in the linkers. However, both
cases converge to the same Kuhn length of $b=___$ in the limit of infinite
variance.
%thoughts on just cutting the harmonic averaging thing, or leaving it for
%discussion? I feel like it's going
%to need more words to make it more clear

% unversality
If we consider the homogenous case to be the minimum entropy arrangement
...
length to which the chains converge by considering the maximum entropy
distribution of nucleosome positions. This corresponds to the case
where nucleosomes are spaced uniformly along the DNA backbone, resulting in
exponentially distributed linker lengths. 
...
As can be seen in FIG 4, the universal behavior of this maximally entropic
polymer allows our model to robustly predict the Kuhn length of chromatin as a
function of just the average linker length in a given organism. The resulting
predictions eliminate the oscillatory dependence of the Kuhn length of
homogenous chromatin chains on the linker length.
Thus, we can conclude that the structure of the homogenous chromatin chain is unbiological, due to it being unstable to perturbations in the nucleosome arrangement.
By isolating the dependence of chromatin's structure on the distribution of
$\phi$, we find that physical properties of the heterogenous chain are
independent of the choice of linker length distribution.

%due to universality, can approximate by WLC with renormalized b
In fact, we find that the short and long length scale behavior of the
heterogenous chain is well-captured by a wormlike chain with a renormalized Kuhn
length. As in FIG 5, we see that the $\RR$ approaches the Gaussian chain regime after just 2
linkers, again due to the fact that $\phi$ is drawn from a uniform
distribution. Thus, mesoscopic models can approximate chromatin as a WLC using
the Kuhn lengths reported in FIG 4. 
...
In the limit of slow nucleosome turnover, in which the positions of nucleosomes
are effectively fixed, we find that the looping probability significantly varies
depending on the particular spacings of intervening nucleosomes.
In particular, we note that the typical model of chromatin as bare DNA with
$lp=50\text{nm}$ systematically underestimates the looping propensity. We find
that heterogenity most significantly affects the looping probability at short
length scales.
...
chains (Supp FIG showing compact and straight homo chains looping). Thus, our
model predicts that in the limit of slow nucleosome turnover, the cell can allow or prevent certain loci from coming into contact by locally rearranging the positions of intervening
nucleosomes.

%fast nucleosome turnover, implications for multiscale modeling
In the limit of fast nucleosome turnover, we can approximate the heterogenous
chain by the average over individual chain configurations, shown in black. 
We note that the kilobase-range peak of the average chain exactly corresponds to the length-scale of enhancer-promoter contacts. 
As can be seen in FIG, our full theory is needed to calculate
the probability of internucleosomal contacts for short chains composed of 3-5
nucleosomes. Howeer, for length scales longer than a kilobase,
the WLC chain with Kuhn length given by our model sufficiently captures the looping probabilities of the average chain obtained from our full theory. At length scales
longer than \SI{1300}{\basepair}, the looping probability approaches that of a
Gaussian chain with the familiar $L^{-3/2}$ scaling.
