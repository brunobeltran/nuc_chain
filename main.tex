...
%TODO change order of methods? how about explaining uniformly distributed
%linkers first
...
Without theory to describe the general features of these heterogenous fibers,
    it has become \textit{de facto} standard to treat chromatin as an effective
    WLC with a persistence length that either matches that of bare
    DNA~\cite{benedetti2017,macphersonInPress,nuebler2018} or is chosen to fit
    the quantity being measured~\cite{sanborn2015,pierro2017}.
...
%TODO add overview of model here
...
\begin{equation}\label{eq:expansion}
    \greens{\vec{k}, \Omega} = \sum_{l m j}\sum_{l_0 m_0 j_0} g_{l_0 m_0 j_0}^{lmj}
        \wigD_l^{mj}(\Omega)\wigD_{l_0}^{m_0 j_0 *}(\Omega_0).
\end{equation}
%TODO incorporate after adding initial explanation of overall model structure to
%beginning of section
% +The final orientation, $\Omega$, can be rotated to account for the
% +    kink introduced the nucleosomes at the end of the linker by replacing
% +    $g$ with $B_{l_{0}m_{0}j_{0}}^{lmj}=\sqrt{\frac{8\pi}{2l+1}}\mathcal{D}_{l}^{jj_{0}}\left(\Omega_{kink}\right)g_{l_{0}m_{0}j_{0}}^{lmj}\left(L\right)$.
% +The coefficients $B_{l_0 m_0 j_0}^{lmj}(\Omega_\text{kink}, L)$ were first computed
% +    in~\cite{zhou2003}, an alternative derivation can be found in the supplement.
...
\begin{figure}[t]
    ...
    }%TODO add dotted parallel line to other end of DNA
...
We extract the ``kink'' rotation $\Omega_\text{kink}$ describing the change in
    ...
    the nucleosome's structure (Figure~\ref{fig:entry-exit}).
...
Doing so does not substantially change the results (see Supplemental Figure
    XX).

In summary, the position and orientation of the $i$th nucleosome exit site
    is given by the convolution
\begin{align*}\label{eq:conv}
\greens{\vec{R},\Omega} &= \prod_{i=1}^{N-1}
    \left[\int d\vec{R}_{i}\int d\Omega_{i}
        G\left(\vec{R}_{i},\Omega_{i}|\Omega_{i-1},L_{i}\right)
    \right]
G\left(\vec{R},\Omega|\Omega_{0};L\right)
    &= \prod_{i=1}^{N-1}\\
    &\cdot G\left(\vec{R}_{N},\Omega|\Omega_{N-1},L_{N}\right),
\end{align*}
where $L = \sum L_i$.
...
Finally, to calculate the probability of two loci looping, we numerical invert
    the Fourier transform to evaluate $\greens{\vec{R}}$ at $\vec{R} = 0$.
