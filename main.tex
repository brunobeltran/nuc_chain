...
We find that heterogenously-spaced kinks in a wormlike chain change the
    ...
    We predict that the Kuhn length of \texit{in vivo} chromatin is up to four times
...
%since the citations are nucleosome positioning papers, should we explicitly say
    ...
    %nucleosomes are heterogenously spaced along DNA?
...
Thus far, detailed computational models of chromatin with random
    ...
    architecture~\cite{woodcock1993,collepardo-guevara2014, bascom2017}.  %we should  probably cite tamar schlick's more recent paper here
...
%    DNA fluctuations~\cite{woodcock1993} 
    ...
    chromatin chains with perscribed nucleosome locations
    ~\cite{collepardo-guevara2014}. As a result, there is
...
In this paper, we present the exact analytical theory for a wormlike chain
    ...
    %should we still be nit-picky about histone binding vs. nucleosome binding?
We show nucleosome binding can increase chromatin elasticity by orders of
    magnitude, and that the amount of this increase depends only on the average linker length for a
    given cell type.
...
Furthermore, our results provide theoretical guidance for quantitatively
    coarse-graining simulations of large-scale chromatin organization from the bottom up~\cite{macphersonInPress}.
...
At each nucleosome, we impose the constraint that the incoming and outgoing
    linker DNA strands have a fixed relative orientation. This relative orientation $\Omega_\text{kink}  =
...
This equation can be solved analytically in Fourier space
    ...
    eigenfunctions of the heat equation on $SO(3)$ ~\cite{spakowitz2006}.
...
Using different values for the wrapping level does not substantially change our
    ...
    %talk to Andy about how deep to go with this.

To compose monomers of the nucleosome chain with perscribed linker lengths, we
perform an iterated convolution of the green's function in equation
\ref{eq:expansion}. The position and orientation of the $n$th nucleosome exit site
    is given by
